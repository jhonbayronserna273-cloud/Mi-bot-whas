...
// ✅ SOLUCIÓN PARA RENDER: Evita el aviso de "No open ports detected"
...
server.listen(PORT, () => {
    console.log(`✅ Servicio auxiliar corriendo en puerto ${PORT}`);
});
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (!msg.key.fromMe && msg.message) {
            ...
            if (comando === 'trabajar') {
                const user = db.get(remitente);
                if (!user) return sock.sendMessage(remitente, { text: '❌ Usa !registrar primero' });
                const gana = Math.floor(Math.random.random() * 60) + 25;
                user.monedas += gana;
                user.xp += 15;
                if (user.xp >= 150) {
                    user.nivel += 1;
